...
const ModifierQuittance = ({ navigation, route }) => {
  const quittance = route.params?.quittance || {};
  ...
  const [email, setEmail] = useState(quittance.email || '');
  const [description, setDescription] = useState(quittance.description || '');
    ...
    try {
      ...
      if (token && userData.codeClient) {
        ...
        if (response.data.success) {
          ...
          navigation.navigate('ListQuittance', { quittanceId: quittance._id });
        } else {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Text style={styles.welcomeText}>Modifier Quittance</Text>
...
export default ModifierQuittance;
